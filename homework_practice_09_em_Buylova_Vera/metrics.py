...
def compute_precision(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> Tuple[int, int]:
    ...
    intersection = sum(len(set(predicted[i]) & (set(ref.possible) | set(ref.sure))) for i, ref in enumerate(reference))
    total_predicted = sum(len(set(pred)) for pred in predicted)
...
def compute_recall(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> Tuple[int, int]:
    ...
    """
    intersection = sum(len(set(predicted[i]) & set(ref.sure)) for i, ref in enumerate(reference))
    total_predicted = sum(len(set(ref.sure)) for ref in reference)
...
def compute_aer(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> float:
    ...
    """
    inter_p, pred_p = compute_precision(reference, predicted)
    inter_r, pred_r = compute_recall(reference, predicted)
    return 1 - (inter_p + inter_r)/(pred_p + pred_r)
