...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        """
        posteriors = [self.translation_probs[pair.source_tokens][:, pair.target_tokens] /
        np.sum(self.translation_probs[pair.source_tokens][:, pair.target_tokens], axis = 0) 
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        """
        elbo = sum([np.sum((np.log(self.translation_probs[pair.source_tokens][:, pair.target_tokens] / 
                                     len(pair.source_tokens) + 1e-8) - np.log(posteriors[i] + 1e-8)) * posteriors[i])
        for i, pair in enumerate(parallel_corpus)])
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        updates = [(np.ix_(pair.source_tokens, pair.target_tokens), posterior) 
                   for pair, posterior in zip(parallel_corpus, posteriors)]
        for indices, posterior in updates:
            np.add.at(self.translation_probs, indices, posterior)
    ...
    def align(self, sentences):
        ...
        for pair in sentences:
            pr = self.translation_probs[np.ix_(pair.source_tokens, pair.target_tokens)]
            source_indices = np.argmax(pr / np.sum(pr, axis = 0, keepdims = True), axis = 0) + 1
            target_indices = np.arange(1, len(pair.target_tokens) + 1)
            alignments.append(list(zip(source_indices.tolist(), target_indices.tolist())))
        return alignments
